...
def custom_score_2(game, player):
    ...
    return float(my_weight * my_moves - opponent_weight * opp_moves)
...
def custom_score_3(game, player):
    ...
    my_distance_from_center_x = abs(center_x - my_position[0])
    my_distance_from_center_y = abs(center_y - my_position[1])

    opp_distance_from_center_x = abs(center_x - opp_position[0])
    opp_distance_from_center_y = abs(center_y - opp_position[1])

    my_weight=1
    opponent_weight=2

    return float(my_weight * my_moves - opponent_weight * opp_moves) - (my_distance_from_center_x + my_distance_from_center_y) + (opp_distance_from_center_x + opp_distance_from_center_y)
...
class MinimaxPlayer(IsolationPlayer):
    ...
    def minimax_decision(self, gameState, depth):
        ...
        for m in legal_moves:
            v = self.min_value(gameState.forecast_move(m), depth-1)
            if v > best_score:
                best_score = v
                best_move = m
        return best_move
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def terminal_test(self, gameState, depth, alpha, beta):
        """ Return True if the game is over for the active player
        and False otherwise.
        """
        if self.time_left() < self.TIMER_THRESHOLD:
    ...
    def min_value(self, gameState, depth, alpha, beta):
        ...
        if self.terminal_test(gameState, depth, alpha, beta):
            return self.score(gameState, self)
        v = float("inf")
        for m in gameState.get_legal_moves():
